...
// Update txId once transaction is passed to the blockchain
const updateTransactionById = asyncHandler(async (req, res) => {
  const transaction = await Transaction.findById(req.body.id);
  if (transaction) {
    transaction.txId = req.body.txId || transaction.txId;
    transaction.manager = req.body.managerId || transaction.manager;
    transaction.status = req.body.status || transaction.status;
    transaction.managerWalletAddress =
      req.body.managerWalletAddress || transaction.managerWalletAddress;
  }
...
//   const { id, txId, managerId, status } = req.body; // after the transaction txId  has been generated by the blockchain
//   // const { orderNo, txId } = req.body; // after the transaction txId  has been generated by the blockchain
...
//     transaction.manager = managerId || transaction.manager;
